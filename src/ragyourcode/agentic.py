...
def research(
    ...
    steps: list[dict] = []
    initial = search(units, query, max(limit, 1), search_index=search_index)
    ...
    expanded = graph_search(units, query, limit=max(limit * 2, 8), hops=hops, graph=graph, search_index=search_index)
